export function loadProducts(productList, load) {
  ...
  productList.forEach((produto) => {
    const valParcela = (parseFloat(produto.preco) / 10).toFixed(2);
    const html = `<div class="product-card idprod" id="${produto.id}">
        <div>
          <img id="${produto.id}"
            src="http://127.0.0.1:8000/${produto.imgProduto}"
        ...
        <div class="product-card-info-container">
          ...
          <h3 class="product-card-price">R$ ${parseFloat(produto.preco).toFixed(2)}</h3>
